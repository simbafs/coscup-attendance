...
export async function getStaticProps() {
  let data = await fetch('http://localhost:3000/session.json').then(res => res.json())
...
export default function Home({ data }) {
  ...
  let [attendance, updateAttendance] = useReducer((curr, update) => {
    let r = {
      29: curr[29],
      30: curr[30],
    }
  ...
  let sessions = groupBy(data.sessions.filter(item => (new Date(item.start)).getDate() == day), 'room')[room]
  // sort by start time
  sessions.sort((a, b) => {
  ...
  return (
    <>
      ...
      {sessions.map(s => <Session
        ...
        setAttendance={n => updateAttendance({
          day: day,
          room: s.room,
          id: s.id,
          attendance: n,
...
function Session({ session, attendance, setAttendance }) {
  return <>
    <div className='w-screen space-x-2'>
      <span>{getFormatedDate(session.start)}-{getFormatedDate(session.end)}</span>
